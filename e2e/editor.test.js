...
test('editor page functionality', async ({page}) => {
  ...
  await expect(page.locator('table .text-input').first()).toContainText('Public Testing')
  expect(await page.inputValue('#rawOutput')).toContain('Public Testing')

  await page.locator('table .text-input').first().fill('Changed! Abcdefghijklmnopqrstuvwxyz')
  await page.locator('body').click()
  await expect(page.locator('table .text-input').first()).toContainText('Changed! Abcdefghijklmnopqrstuvwxyz')
  ...
  await page.locator('table .text-input').last().fill('other change')
  await page.locator('body').click()
  await expect(page.locator('.num-changes')).toContainText('2 changes')
  await page.locator('table .text-input').last().fill('Submit')
  await page.locator('table .text-input').first().fill('Public Testing')
  await page.locator('body').click()
  await expect(page.locator('.num-changes')).not.toBeVisible()

  await page.locator('table .text-input').last().fill('to be saved')
  ...
  await page.locator('table .text-input').first().fill('Will stay after changing language')
